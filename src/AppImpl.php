...
final class AppImpl implements App {
    ...
    private function isHtml(Request $request = NULL) : bool {
        if (!\is_null($request)) {
            ...
            if (\array_key_exists(AppImpl::KEY_ACCEPT_TYPE, $headers)) {
                return (\stripos($headers[AppImpl::KEY_ACCEPT_TYPE], AppImpl::VALUE_TYPE_HTML) >= 0);
            } else {
                return false;
            }
        } else {
